...
data = {'Word': stp_words, 'Pos': np.asarray(pos_counts)/tot_pos, 'Neg': np.asarray(neg_counts)/tot_neg}
df = pd.DataFrame(data,columns=['Word', 'Pos','Neg'], index = stp_words)

sns.scatterplot(data=df, x='Pos', y='Neg')
plt.xlabel('Positive')
plt.ylabel('Negative')
